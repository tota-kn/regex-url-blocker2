...
- `dailyRules` は曜日別の制限ルール。通常は0=日曜から6=土曜まで7件を保存する。
...
- 旧 `schedules` / `blockedTimeSlots` / `timeLimits` は `dailyRules` へ移行しない。`dailyRules` 欠損時は空の `dailyRules` を生成し、旧フィールドは破棄する。
- `dailyRules` が配列でない場合は空の `dailyRules` を生成する。配列の場合は各要素を `DailyRule` 形状へ正規化する。
...
- export は常に `version: 3` を出力する。
- import は互換のため `version: 2` と `version: 3` のみ受け付ける。
...
- v3 import では、グループ側の `blockAction` / `redirectUrl` を保持する。
