...
- `restrictionRules` は条件付きの制限ルール配列。空配列は制限なし。詳細は `docs/spec/domain.md`「グループ」節を参照。
...
- `restrictionRules` の正規化：
  - `restrictionRules` が配列の場合は各要素を正規化する。`condition.type` が既知の4種（`daily` / `weekly` / `monthly` / `period`）以外の要素、または `type` が `block` / `grace` / `wait` 以外の要素は破棄する。
  - 旧 `restriction` 単一値がある場合は `restrictionRules: [restriction]` へ変換する。
  - 旧 `scheduleRules` がある場合は、各ルールの `blockedTimeRanges` を `block`、`dailyLimitMinutes` を `grace`、`delaySeconds` を `wait` の制限ルールへ展開し、block → grace → wait の順に並べる。
  - `restrictionRules` / 旧 `restriction` / 旧 `scheduleRules` が無く、旧 `dailyRules`（曜日別ルール配列）がある場合は、旧 `scheduleRules` 相当にまとめたうえで `restrictionRules` へ変換する。
  - それ以前の旧 `schedules` / `blockedTimeSlots` / `timeLimits` しかない場合は空の `restrictionRules` を生成し、旧フィールドは破棄する。
...
- export は常に `version: 8` を出力する。
- import は互換のため `version: 2` / `3` / `4` / `5` / `6` / `7` / `8` を受け付ける。
...
- v2〜v4 import では、グループ側の旧 `dailyRules` を上記の正規化規則で `restrictionRules` へ自動変換する。
- v5〜v7 import では、グループ側の旧 `scheduleRules` または旧 `restriction` を `restrictionRules` へ自動変換する。
