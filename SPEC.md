...
- **スケジュール** — グループの閲覧条件1組。曜日・時間帯・1日あたり上限分数を一体で持つ。空配列なら 24 時間・上限なしで閲覧可能。
...
- グループ単位の判定式：
  - `schedules` が空配列なら、そのグループは常に許可状態（24 時間・上限なし）
  - `schedules` が非空のとき、現時点の (曜日 × 時刻) が **いずれの** schedule にもマッチしなければブロック状態
  - マッチしたスケジュール群の `dailyTimeLimitMinutes` のうち **最も厳しい値**（最小）を有効上限とする。`null` は「上限なし=∞」とみなすため、数値が1つでもあればその最小、すべて `null` ならば上限判定なし。`consumedSec >= 有効上限 * 60` ならブロック状態
- URL 単位の判定：
...
- 過去日の履歴は保持しない（今日分のみ）。
- 日跨ぎのスケジュールは `end <= start` で表現する（例 `22:00–06:00`）。
...
- グループの追加・編集・削除
- 各グループ：`name`、`patterns[]`、`schedules[]`
- スケジュールは1組につき：曜日チェックボックス（日〜土の7つ。未選択=毎日）、`<input type="time">` の開始/終了、上限分数の `<input type="number">`（空=上限なし）
...
- グループごとに：`name`、`消費 / 上限`（`M:SS / 30:00` 形式）、許可スケジュール内/外バッジ、ブロック発動中バッジ
- 全体のリセットまでのカウントダウン
- 「設定を開く」リンク
